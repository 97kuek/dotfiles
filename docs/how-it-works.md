...
| 種類 | 例 | 書いてある場所 | 揃える方法 |
| --- | --- | --- | --- |
| 設定ファイル | `.zshrc`、`.gitconfig` | `zsh/`、`git/`など | GNU Stowで`~`にリンクする |
| ツールとアプリ | starship、Ghostty、Codex | `Brewfile` | Homebrewで入れる |
...
├── zsh/                ┐
├── git/                │  設定ファイル
├── starship/           │  （Stowのパッケージ。中身がそのまま ~ にリンクされる）
├── ghostty/            │
├── ssh/                ┘
│
...
- 入れているもの
  - **開発ツール**：gh、jq、node、pyenv、uv、volta、stow、tmux、cocoapods
